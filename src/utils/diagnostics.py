...
class NetworkDiagnostics:
    ...
    def test_port_connectivity(self, host: str, port: int, 
                              timeout: int = 5) -> DiagnosticResult:
...
class ConfigurationDiagnostics:
    ...
    def validate_configuration_files(self) -> DiagnosticResult:
        ...
        try:
            ...
            if not Path(logging_config.file_path).parent.exists():
                issues.append(f"Logging directory does not exist: {Path(logging_config.file_path).parent}")
            ...
            try:
                ...
                if market_data_config and hasattr(market_data_config, 'enabled') and market_data_config.enabled:
            ...
            message = "Configuration validation passed" if not issues else f"Found {len(issues)} configuration issues"
    ...
    def test_file_permissions(self) -> DiagnosticResult:
        ...
        try:
            ...
            status = DiagnosticStatus.PASS if not issues else DiagnosticStatus.WARNING
            message = "File permissions OK" if not issues else f"Found {len(issues)} permission issues"
...
class PerformanceDiagnostics:
    ...
    def check_system_resources(self) -> DiagnosticResult:
        ...
        try:
            ...
            message = "System resources OK" if not issues else f"Found {len(issues)} resource issues"
    ...
    def performance_benchmark(self) -> DiagnosticResult:
        ...
        try:
            ...
            status = DiagnosticStatus.PASS if not issues else DiagnosticStatus.WARNING
            message = "Performance benchmark completed" if not issues else "Performance issues detected"
...
class SystemDiagnosticsManager:
    ...
    def run_full_diagnostics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                market_data_config = config_manager.get_agent_config('market_data')
                if (market_data_config and 
                    hasattr(market_data_config, 'enabled') and market_data_config.enabled and
                    hasattr(market_data_config, 'url')):
                    import urllib.parse
                    parsed = urllib.parse.urlparse(market_data_config.url)
                    results.append(self.network_diagnostics.test_port_connectivity(
        ...
        self.logger.info(f"Diagnostics completed: {overall_status} ({passed}/{total_tests} passed)")
        return summary
    ...
    def _check_dependencies(self) -> DiagnosticResult:
        ...
        try:
            ...
            status = DiagnosticStatus.PASS if not issues else DiagnosticStatus.WARNING
            message = "All dependencies satisfied" if not issues else f"Found {len(issues)} dependency issues"
