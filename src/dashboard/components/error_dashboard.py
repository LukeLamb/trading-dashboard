...
from ..utils.error_handling import (
    ...
    RecoveryStrategy, DiagnosticStatus
)
            ...
            if stats.get("error_by_severity"):
                ...
                fig = go.Figure(data=[
                    go.Bar(
                        x=severities,
                        y=severity_counts,
                        marker_color=[colors.get(s.lower(), '#999999') for s in severities]
                    )
                ])
                fig.update_layout(
...
def render_system_diagnostics():
        ...
        if st.button("🚀 Run Full Diagnostics", type="primary"):
            with st.spinner("Running comprehensive system diagnostics..."):
                try:
                    ...
                    st.success(f"Diagnostics completed! Status: {results['overall_status']}")
    ...
    if 'diagnostic_results' in st.session_state:
        ...
        with col3:
            st.metric("Failed", summary['failed'], delta="❌" if summary['failed'] > 0 else None)
        with col4:
            st.metric("Warnings", summary['warnings'], delta="⚠️" if summary['warnings'] > 0 else None)
        with col5:
            st.metric("Errors", summary['errors'], delta="💥" if summary['errors'] > 0 else None)
        with col6:
            ...
            with st.expander("🖥️ System Information"):
                ...
                with col2:
                    ...
                    if sys_info['disk_space']:
                        st.write(f"• Disk Total: {sys_info['disk_space'].get('total', 'N/A')}")
                        st.write(f"• Disk Free: {sys_info['disk_space'].get('free', 'N/A')}")
...
def render_live_monitoring():
    ...
    if auto_refresh:
        ...
        try:
            ...
            st.warning("Install psutil for live system monitoring: `pip install psutil`")
...
def render_debug_tools():
    ...
    if st.session_state.debug_mode:
        st.info("Debug mode is enabled. Additional information will be shown throughout the application.")
        ...
        with st.expander("🧪 Error Simulation (For Testing)"):
            ...
            error_type = st.selectbox(
                "Error Type",
                ["ValueError", "ConnectionError", "KeyError", "TypeError", "RuntimeError"]
            )
            
            error_message = st.text_input(
                "Error Message",
                value="Test error for debugging purposes"
            )
